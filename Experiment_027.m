...
sca;
%  Version info
Version = 'Experiment_027_v.1.6' ; % after code changes, change version
file_directory='C:\Jackson\Adriana Stuff\A&V_Experiment_Jack_062122';
...
addpath('C:\Jackson\Adriana Stuff\A&V_Experiment_Jack_062122\Auditory Stimulus');
...
check_go = 1;
file_name_check = [file_directory '\Behavioral_Data\' save_name '.mat'];
...
aud_time_frames = round((audInfo.dur+audInfo.silence)/ ifi);
...
while (BreakState ~= 1) && (block_counter <= total_blocks) % each block
    ...
    dataout(output_counter,1:6) = {'Trial #' 'Position #' 'Fixation Correct' 'AV Reward' 'Target Correct' 'Total Trial Time (sec)'}; %Initialize Columns for data output cell

    
    while (trialcounter <= total_trials) && (BreakState ~= 1) % each trial
        output_counter = output_counter + 1;
        start_trial_time = GetSecs; %Trial Start Time
        ...
        [audInfo.CAM] = makeCAM(audInfo.coherence, audInfo.dir, audInfo.dur, audInfo.silence, 44100);
        [audInfo.adjustment_factor, CAM_1, CAM_2] = Signal_Creator(audInfo.CAM,audInfo.dB); %Writes to .f32 files for .rcx circuit to read

                
        TDT.write('mux_sel',audInfo.mux); %The multiplexer values for each trial, set to all zeros for now to include only LR and RL
        TDT.write('dur',audInfo.dur); %duration of the stimulus
        TDT.write('adjustment_factor',audInfo.adjustment_factor);%For dB adjustment variables to the circuit
        TDT.write('CAM_1',CAM_1); %Signal 1 
        TDT.write('CAM_2',CAM_2); %Signal 2
...
%             end
        ...
        % points, IN PROGRESS
        targ_timeout = 0;
        if fix_timeout ~= 1 && av_timeout ~= 1 && baron_fixation_training ~= 1
        ...
        dataout(output_counter,1:6) = {trialcounter pos fix_reward av_reward target_reward trial_time}; 
...
%% End of Block 
    ...
    %Count all of the AV rewards given and divde by total trials to get a AUD Success Rate    
    av_rew = zeros(1,total_trials); 
    for h = 2:total_trials+1
        if strcmp(dataout{h,4},'Yes')
            av_rew(h-1) = 1;
        end
    end
    AV_Success_Rate = sum(av_rew)/total_trials

    %Count all of the target rewards given and divde by total trials to get a Target Success Rate 
    targ_rew = zeros(1,total_trials); 
    for k = 2:total_trials+1
        if strcmp(dataout{k,5},'Yes')
            targ_rew(k-1) = 1;
        end
    end
    Target_Success_Rate = sum(targ_rew)/sum(av_rew)
    
    
    
    times = cell2mat(dataout(2:end,6)); %Extract the trial times 
